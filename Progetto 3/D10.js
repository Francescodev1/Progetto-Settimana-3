...
let eliminaPrima = deleteOne('HELLO', true);
console.log(eliminaPrima);
let eliminaUltima = deleteOne('HELLO', false);
...
function onlyLetters (a){
  ...
  let risul = onlyLetters("I have dogs");
...
if(a.includes('@') && a.indexOf('@') === a.lastIndexOf('@') && a.includes(".")>0){
  return true;
} else {
  return false;}
}

let email = isThisAnEmail('ciao');
console.log(email);
email = isThisAnEmail('frank@libero.it');
console.log(email);
...
function rollTheDices(a) {
  ...
  for (let i = 0; i < a; i++) {
    let valori = dice();
    resul.sum += valori;
    resul.values.push(valori);
  }
  return resul;
}

const finalResult = rollTheDices(3);
...
*/
function isTodayMyBirthday(giornoCompleanno){
  let today = new Date();
  let giornoCompleannoobj = new Date(giornoCompleanno);
  let mese = today.getMonth() === giornoCompleannoobj.getMonth();
  let giorno = today.getDay() === giornoCompleannoobj.getDay();
  return mese && giorno;
}


let mioCompleanno = '2024-03-17 '; 
let result = isTodayMyBirthday(mioCompleanno);
if (result) {
...
const resultDelete = deleteProp(obj, 'city');
...
const risultato3 = searchByTitle('lord');
...
const result4 = searchAndDivide('of');
console.log(result4);
/* ESERCIZIO 19
...
function addElem (){
  ...
  newEle.innerText = 'Nuovo elemento'
  let lista = document.getElementById('myList');
  lista.appendChild(newEle);
}
addElem()
